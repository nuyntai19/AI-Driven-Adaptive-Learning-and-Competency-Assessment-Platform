...
export interface ScratchpadCanvasModalProps {
  ...
  onExportPng?: (png: Blob | null) => void;
}
...
export const ScratchpadCanvasModal = ({
  ...
  const close = useCallback(() => {
    if (isDraftLoaded) {
      ...
      if (onExportPng) {
        if (current.strokes.length > 0) {
          void renderPng(current.strokes, current.grid, CANVAS_WIDTH, CANVAS_HEIGHT)
            .then((blob) => {
            ...
            .catch(() => {
              // Ignore canvas export failures during modal close
            });
        } else {
          onExportPng(null);
        }
    ...
    onClose();
  }, [isDraftLoaded, onClose, onExportPng, saveDraft]);
    ...
    >
      <section className="flex h-full w-full flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
        ...
        <footer className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 bg-slate-50 px-4 py-2 text-xs text-slate-600">
          ...
          <span>Bản vẽ nháp tự động lưu và đính kèm vào bài nộp khi hoàn thành.</span>
        </footer>
      </section>
    </div>
  );

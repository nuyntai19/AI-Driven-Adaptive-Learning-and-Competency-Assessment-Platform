...
| **Checkpoint 8** | `feat(center-manager): complete dynamic rbac and authorization audit ux` | Phần 2 (H) | **COMPLETED (CORRECTIVE VERIFIED)** | Hoàn thiện toàn diện Giai đoạn H (Dynamic RBAC & Authorization Audit UX) kèm khắc phục triệt để 7 phát hiện audit từ Codex: (1) Chuẩn hóa tracking: gán đúng nhãn Giai đoạn H cho RBAC; (2) Triển khai server-side search/filter/pagination cho danh sách vai trò (`page`, `pageSize`, `search`, `accountType`, `status`), loại bỏ giới hạn 100 bản ghi trên client; (3) Bổ sung DTOs, UseCase `ListAuthorizationUsersUseCase` và endpoint `GET /api/v1/authorization/users` hỗ trợ tìm kiếm và phân trang toàn bộ người dùng trong trung tâm (`CenterManager` gồm cả secondary managers, `Teacher`, `Student`); (4) Hoàn thiện đầy đủ bộ lọc H3 cho nhật ký kiểm toán: `ActorUserId`, `TargetUserId`, `TargetId`, `PermissionCode`, `ActionType`, `From`, `To` cùng nút đặt lại bộ lọc; (5) Bổ sung bộ kiểm thử 25 bài tại `authorizationManagement.test.ts` (113/113 web tests pass), kiểm tra toàn diện helper, query builders, sanitization và logic phân quyền; (6) Triển khai `parseSafeError` triệt tiêu nguy cơ rò rỉ raw `ProblemDetails.detail`/`title` của backend, chỉ trả thông báo an toàn mặc định và `traceId`; (7) Nghiêm ngặt tuân thủ không in credential ra transcript/git; (8) Backend 241/241 IdentityAndTenancy tests pass, frontend 113/113 tests pass, ESLint 0 warning/0 error, production build pass. |

...
| **Checkpoint 7** | `feat(center-manager): complete custom role and rbac matrix ux` | Phần 2 (G) | **COMPLETED (CORRECTIVE VERIFIED / TECHNICALLY APPROVED)** | Hoàn thiện toàn diện Custom Role & Dynamic RBAC Matrix UX (Giai đoạn G): (1) Quản lý vòng đời vai trò tùy chỉnh: tìm kiếm, lọc theo Loại tài khoản (AccountType) và Trạng thái (Status), modal tạo vai trò với mã vai trò (uppercase alphanum), tên vai trò, loại tài khoản tương thích, mô tả; (2) Ma trận quyền truy cập động: phân nhóm theo module, nút chọn/bỏ chọn tất cả theo module, hiển thị granular badges ('Bạn có', 'Nhạy cảm', 'Không tương thích', 'Không thể ủy quyền', 'Vượt thẩm quyền'), bảo vệ System Role read-only với banner thông báo, lưu ma trận quyền Canonical kèm bắt buộc lý do (Reason); (3) Gán vai trò người dùng (User Role Assignment): tìm kiếm người dùng, lọc loại tài khoản, hiển thị các vai trò tương thích đang kích hoạt, banner Cảnh báo tự thay đổi vai trò (Self-change Warning) khi người dùng hiện tại được chọn, bảng Quyền hạn hiệu lực (Effective Permissions) hiển thị nguồn gốc vai trò (← RoleName); (4) Nhật ký kiểm toán phân quyền (Authorization Audit): bộ lọc theo Hành động (ActionType) và khoảng thời gian (Date range), phân trang, sao chép 1-click Trace ID, Modal so sánh Before/After chi tiết bằng JSON diff; (5) Xử lý OCC 409 khi có cập nhật tranh chấp và guards ngăn chặn gửi form khi không có thay đổi (no-op submit guards); (6) Kiểm chứng: 16 bài kiểm thử tự động tại authorizationManagement.test.ts (104/104 web tests pass), ESLint 0 error/0 warning, Web production build sạch (14.49s), 238/238 backend IdentityAndTenancy tests pass, 3.369 backend unit tests pass, kiểm chứng E2E thành công trên trình duyệt Chrome thực tế với tài khoản CenterManager xác thực qua Docker. |

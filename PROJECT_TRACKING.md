...
| **Checkpoint 2** | `test(center-manager): lock tenant and account-type boundaries` | Phần 1 (B) | **IN PROGRESS** | Viết bộ kiểm thử hồi quy bảo mật: CenterManager bị từ chối khỏi `/api/v1/platform/*` và `/quan-tri-nen-tang/*`; Teacher không có center-wide scope ngầm; Custom CenterManager role bị giới hạn theo effective permission; Cross-tenant identifiers fail-closed 404; Account-type compatibility, self-elevation, over-grant và last-admin invariant được bảo vệ. |
| **Checkpoint 3** | `feat(center-manager): complete center profile and dashboard UX` | Phần 1 (C) | **PENDING** | Bổ sung frontend API `getCurrentCenter()`, `updateCurrentCenter(request)`; trang CenterProfilePage tại `/quan-ly/trung-tam` (read-only code/status, editable name/timezone, OCC RowVersion); audit CenterDashboardPage (subject filter, N+1 elimination, charts fallback). |
| **Checkpoint 4** | `feat(center-manager): complete teacher and student account lifecycle` | Phần 1 (D) | **PENDING** | Hoàn thiện vòng đời Teacher (detail, update, soft-delete với active class guard, reset password an toàn) và Student (detail, update, xem lớp, subject goal, soft-delete bảo toàn 100% evidence/twins/assignments/attempts với `organization.students.delete`, password reset an toàn với `organization.teachers.reset_password` & `organization.students.reset_password`). |
| **Checkpoint 5** | `feat(center-manager): complete class management lifecycle` | Phần 2 (E) | **DEFERRED (PHẦN 2)** | Chuyển sang thực hiện tại Phần 2 theo kế hoạch authoritative. |

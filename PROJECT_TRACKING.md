...
| **GATE 5** | Track 2D — Multipart Storage, Bảng vật lý 40, Gemini Multimodal & Review Fallback | **FROZEN** | Đóng băng kỹ thuật chính thức (FORMAL FREEZE) Gate 5: Bảng vật lý thứ 40 `attempt_attachments`, `IAttemptTeacherReviewScopeGuard` fail-closed, upload PNG bounded multipart ingestion (buffer ≤5 MB) kèm strict chunk validation, Data Protection signed token 24h & unique nonce race resolution (HTTP 409), 0..3 retries state machine với MySQL CHECK constraint `ck_ai_analysis_jobs_retry_count` và `ck_attempts_status` (`AnalysisFailed`), polling wire contract `attemptStatus`/`errorCode`, resubmit cho bài free-practice bảo toàn strokes nháp và persist `clientSubmissionId` mới vào `sessionStorage` chống ID drift qua reload. Bằng chứng kiểm thử local (Local Execution Evidence — GitHub CI workflow tồn tại nhưng chưa có run attestation cho freeze commit và chưa cover Node/MySQL): Backend thừa kế từ commit `9e76934` (3,208/3,208 non-MySQL tests pass, 45/45 live MySQL integration tests pass trên port 3307), Frontend tại commit `81bcc1e` (46/46 web tests pass, eslint 0 error, build production clean, git diff check sạch). |

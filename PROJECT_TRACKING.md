...
| 2026-09-12 | 3.0.5-gate2-frozen-final | Gate 2: Final Freeze & Audit Correction (Relational Interceptor Concurrency Race, Consecutive Reset OCC, Manager Search Filter, Web 12-char Policy, API Contracts Sync) | Khắc phục triệt để các phát hiện audit vòng 2 và 3: (1) Cập nhật API_CONTRACTS.md Contract 75 và 76 bổ sung InitialManagerUserRowVersion; (2) Khắc phục lỗi OCC UI: bổ sung InitialManagerUserRowVersion vào contract PlatformCenterDetailDto/PlatformCenterListItemDto, loại bỏ hoàn toàn fallback "1" trên UI (fail-closed refetch nếu thiếu version), giải quyết dứt điểm lỗi reset mật khẩu liên tiếp bị 409; (3) Triển khai test concurrency quan hệ thực thụ: sử dụng DbCommandInterceptor trên live MySQL container mô phỏng race condition vật lý ném DbUpdateConcurrencyException và MySQL 1062 duplicate key; (4) Khóa chặt logic bắt duplicate 1062 bằng phép AND bắt buộc khớp index ux_centers_center_code, bổ sung negative test kiểm chứng 1062 trên index khác bị rethrow nguyên vẹn; (5) Bổ sung integration test tuần tự 1 -> 2 -> 3 -> stale 409; (6) Bổ sung tìm kiếm trung tâm theo username và display_name của manager trước phân trang; (7) Enforce minLength 12 ký tự cho mật khẩu trên UI và cập nhật placeholder; (8) Loại bỏ hoàn toàn fallback password tĩnh trong mọi tài liệu. Kết quả kiểm thử local có kiểm chứng: 3,121/3,121 .NET tests pass (0 failed, 0 skipped!), 14/14 live MySQL integration tests pass, 21/21 Node.js test runner tests pass, eslint 0 error, web build pass, EF model up-to-date (0 pending changes). | Nhóm EduTwin | GATE 2 FROZEN; 0 skipped; 2 file DOCX bảo tồn nguyên vẹn |

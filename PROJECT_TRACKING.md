...
| **GATE 4** | Track 2C — Vector Scratchpad Canvas & Scoped IndexedDB Cache | **FROZEN** | Hoàn thành `ScratchpadCanvasModal.tsx` toàn màn hình với bút, tẩy, thước thẳng, compa/tròn, tam giác, trục Oxy, lưới toán/ô ly và history Undo/Redo tối đa 30 trạng thái. Draft chỉ lưu vector strokes trong IndexedDB theo khóa bắt buộc `draft:{centerId}:{userId}:{clientSubmissionId}`; khởi động dọn TTL 24 giờ, logout dọn đúng scope user, và nộp bài chỉ xóa draft sau HTTP 202/200. PNG được xuất từ vector trong bộ nhớ, xác thực giới hạn 5 MB; chưa upload hay thay đổi backend/schema (Gate 5). Xác minh: 34/34 web tests pass, eslint 0 errors/warnings, production build pass. |

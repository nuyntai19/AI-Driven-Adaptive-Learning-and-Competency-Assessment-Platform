...
| 2026-09-15 | 3.0.24-post-r09-phase-e-corrective-pass | `ClassesController.cs`, `IListClassCandidateStudentsUseCase.cs`, `ListClassCandidateStudentsUseCase.cs`, `StudentListQuery.cs`, `ClassesControllerTests.cs`, `ListStudentsUseCaseTests.cs`, `ListClassCandidateStudentsUseCaseTests.cs`, `ClassListPage.tsx`, `classListHelpers.ts`, `classListPage.test.ts`, `API_CONTRACTS.md`, `UI_UX_SPEC.md`, `PROJECT_TRACKING.md` | Khắc phục toàn diện 5 phát hiện audit P1/P2 của Codex cho Giai đoạn E: (1) Checkbox toggle: triệt tiêu double toggle bằng `onClick={(e) => e.stopPropagation()}` trên input; (2) Incomplete member exclusion: triển khai endpoint `GET /api/v1/classes/{classId}/candidate-students` với use case `IListClassCandidateStudentsUseCase` và SQL anti-join `!_dbContext.ClassStudents.Any(cs => cs.ClassId == classId && cs.Status == ClassStudentStatus.Active)`, loại trừ triệt để 100% thành viên lớp ở DB level xuyên suốt mọi trang; (3) 100 candidate cap: hỗ trợ phân trang và tìm kiếm phía máy chủ (`CandidateStudentListQuery`), bảo toàn danh sách tuyển chọn (`selectedStudentIds`) qua các trang; (4) Dynamic RBAC dependency gating: phân tách độc lập `canUpdateClass` (cần classes.update AND teachers.read), `canAddMembers` (cần classes.manage_members AND students.read), `canRemoveMembers` (chỉ cần classes.manage_members); (5) Candidate error state: phân biệt lỗi truy vấn (banner cảnh báo kèm nút Thử lại) với danh sách rỗng, đồng thời chuẩn hóa Active-only constraint trên cả candidate query và BLL thêm học sinh; (6) Bộ kiểm thử hồi quy: 846 backend tests pass (13 test mới), 84 web tests pass (4 test mới), eslint 0 error, production build pass (33s), EF model 0 drift. | Nhóm EduTwin / Codex | GIAI ĐOẠN E CORRECTIVE VERIFIED; 846 Backend + 84 Web tests pass |
| 2026-09-15 | 3.0.25-post-r09-phase-g-hardening | `CurriculumsController.cs`, `CurriculumGuards.cs`, curriculum/question use cases and tests, `CurriculumEditorPage.tsx`, `QuestionEditorPage.tsx`, `curriculumEditorHelpers.ts`, `curriculumHardening.test.ts`, `API_CONTRACTS.md`, `PROJECT_TRACKING.md` | Hoàn tất audit và corrective Giai đoạn G: (1) khóa Teacher ownership cho get/update/replace classes/replace nodes/publish Curriculum, trong khi CenterManager vẫn tenant-wide theo capability; (2) chuẩn hóa RowVersion ASCII dương, 409 cho stale OCC và invalid state, Draft-only mutation; (3) atomic replace classes/nodes, từ chối duplicate, class sai Subject/inactive và node sai Subject/inactive; (4) loại bỏ mọi auto-select Teacher/CenterManager-as-Teacher khi tạo Curriculum hoặc Question, yêu cầu CenterManager chọn active Teacher cùng tenant; (5) Question delete chỉ cho CenterManager, chỉ Draft chưa có Assignment/Attempt, soft-delete Question/Options và bảo toàn lịch sử; (6) Assignment flow được audit, xác nhận draft target, atomic publish snapshot, RowVersion close và center-safe progress đã có test bao phủ; (7) frontend thêm Teacher selector, publish confirmation, OCC refetch và safe error handling. Verification: solution build 0 warning/error; 324 targeted BLL pass + 2 live MySQL pass; 3.416 non-MySQL pass; 127 frontend pass; ESLint sạch; production build pass; `git diff --check` sạch. | Nhóm EduTwin / Codex | GIAI ĐOẠN G CORRECTIVE VERIFIED / TECHNICALLY APPROVED |
| 2026-09-18 | 3.0.26-student-subsystem-modernization | `StudentLayout.tsx`, `StudentDashboardPage.tsx`, `StudentAssignmentsPage.tsx`, `StudentAssignmentDetailPage.tsx`, `StudentTwinPage.tsx`, `LearningPlayerPage.tsx`, `SideAssistantWorkspace.tsx`, `CasioInlinePanel.tsx`, `ScratchpadInlinePanel.tsx`, `FunctionGrapher.tsx`, `VisualMathField.tsx` | Tái thiết kế toàn diện 5 trang chính phân hệ học sinh và không gian trợ lý toán học theo thiết kế Bento Grid và chuẩn mực giao diện cao cấp: (1) Tạo `StudentLayout.tsx` đóng vai trò shell điều hướng với Tabs, bộ chọn môn học toàn cục đồng bộ query params `?subjectId=...`, chuỗi ngày học tập (`Streak Pill`), ThemeToggle sáng/tối; (2) Bento Grid Dashboard với 4 thẻ KPI chỉ số năng lực, biểu đồ Radar/Line và banner Đề xuất học tập AI có thể giải thích kèm nút chấp nhận và nút bỏ qua; (3) Assignments & Detail với bộ lọc tìm kiếm, tabs trạng thái và thanh tiến trình hoàn thành sinh động; (4) Hồ sơ năng lực số Digital Twin với Tăng trưởng nhận thức, Hiệu chuẩn tự tin, Bảng chủ đề tri thức và Dòng thời gian lịch sử cập nhật có phân trang; (5) Learning Player tích hợp Dual-pane workspace với SideAssistant (Casio fx-580VN X chèn kết quả 1-chạm, Bảng nháp Vector có IndexedDB và xuất ảnh minh chứng, Đồ thị hàm số 2D) và trình soạn thảo MathLive WYSIWYG (`VisualMathField`). | Nhóm EduTwin | STUDENT SUBSYSTEM MODERNIZATION COMPLETED; production build clean |
| 2026-09-18 | 3.0.27-fix-player-react-hook-violation | `LearningPlayerPage.tsx`, `PROJECT_TRACKING.md` | Khắc phục triệt để lỗi vi phạm thứ tự React Hook (`Minified React error #310`): Di chuyển toàn bộ các khai báo Hook (`useMemo`, `useState`, `useEffect`) lên vị trí trên cùng của component trước bất kỳ câu lệnh trả về có điều kiện (`if (loading) return`, `if (jobPolling) return`), bảo đảm tính bất biến về số lượng và thứ tự hook qua mọi chu kỳ kết xuất. Build production sạch sẽ 0 lỗi. | Nhóm EduTwin | RULES OF HOOKS HARDENING COMPLETED |
| 2026-09-18 | 3.0.28-gemini-multi-key-pool-failover | `GoogleGenAIGenerateContentClient.cs`, `GeminiOptions.cs`, `docker-compose.yml`, `README.md`, `PROJECT_REQUIREMENTS.md` | Nâng cấp hạ tầng AI Gemini với cụm 3 khóa đa tầng (`Gemini__ApiKey`, `Gemini__BackupKeys`, `Gemini__BackupKeys_2`): triển khai cơ chế Round-Robin phân bổ tải đều qua atomic counter và tự động chuyển đổi khóa ngay lập tức (Automatic Failover) khi gặp lỗi quota (HTTP 429), timeout hoặc sự cố mạng; bảo đảm tính bền bỉ và không gián đoạn cho học sinh làm bài. | Nhóm EduTwin | GEMINI MULTI-KEY POOL & FAILOVER COMPLETED |
...
| **Checkpoint 5** | `feat(center-manager): complete class management lifecycle` | Phần 2 (E) | **COMPLETED (CORRECTIVE VERIFIED / TECHNICALLY APPROVED)** | Hoàn thiện toàn diện quản lý lớp học và thành viên lớp kèm khắc phục triệt để 5 phát hiện audit: (1) Capability gating và dynamic dependency: classesRead, canCreateClass, canUpdateClass (classes.update + teachers.read), canAddMembers (classes.manage_members + students.read), canRemoveMembers (classes.manage_members), dashboardsCenterRead/dashboardsTeacherRead; (2) ClassListPage tích hợp actions column (Chi tiết, Sửa, Dashboard); (3) ClassDetailModal hiển thị thông tin lớp và danh sách thành viên kèm tìm kiếm/phân trang; (4) EditClassModal sửa tên lớp, giáo viên, trạng thái bằng RowVersion, môn học/năm học bất biến theo Contract 32; (5) AddStudentsModal tuyển chọn ứng viên qua endpoint chuyên dụng `GET /classes/{classId}/candidate-students`: SQL anti-join loại trừ 100% thành viên lớp ở DB level (không giới hạn trang hiện tại), phân trang/tìm kiếm phía server gỡ bỏ giới hạn 100 học sinh, bảo toàn tuyển chọn qua các trang, banner lỗi kèm nút thử lại, nghiêm ngặt Active-only constraint; (6) RemoveStudentModal xóa mềm thành viên, chuyển trạng thái Removed và bảo toàn 100% attempts/evidence; (7) Khắc phục lỗi checkbox double-toggle bằng stopPropagation; (8) 15 tests tại classListPage.test.ts (84/84 web tests pass), 857 backend organization tests pass (5 skipped MySQL live tests / 862 total), eslint 0 error, build production pass, API_CONTRACTS.md đã thống nhất empty classId về 404 RESOURCE_NOT_FOUND đồng bộ fail-closed với controller. |
| **Checkpoint 6** | `feat(center-manager): complete subject and knowledge graph lifecycle` | Phần 2 (F) | **COMPLETED (CORRECTIVE VERIFIED / TECHNICALLY APPROVED)** | Hoàn thiện Subject lifecycle (list active/inactive, search, create, detail qua API canonical, update mã/tên/mô tả/trạng thái bằng RowVersion, soft-delete với dependency/evidence 409) và Knowledge Graph lifecycle (create/update/delete node/edge theo capability, cycle/dependency errors, atomic query refresh). Khắc phục phần Gemini dở dang: nối route/menu `/quan-ly/mon-hoc`; bổ sung editable fields + canonical RowVersion vào graph response thay vì client fallback `"1"`; sửa `examImportance` về 0..100 và thời lượng tối thiểu 1; OCC conflict tự nạp phiên bản mới. Verification: build .NET 0 warning/error; 813 KnowledgeGraph tests pass; full backend 3,369 pass, 56 live-MySQL tests skipped do không cấp admin test connection trong run thường; frontend 88/88 pass, ESLint sạch, production build pass; Docker API/Web rebuild và live browser smoke xác nhận CenterManager xem Subject detail, mở graph và mở form edit node/edge với dữ liệu canonical. |
| **Checkpoint 7** | `audit(center-manager): audit curriculum, question bank and assignment hardening` | Phần 2 (G) | **COMPLETED (CORRECTIVE VERIFIED / TECHNICALLY APPROVED)** | Hoàn tất audit Curriculum, Question Bank và Assignment: Teacher ownership/CenterManager tenant scope, Draft-only mutation, atomic class/node replacement, canonical OCC 409, explicit active Teacher ownership khi CenterManager tạo Curriculum/Question, Question delete state/dependency guard và zero student answer leakage. Assignment create/update/target summary/publish snapshot/close/progress được đối soát và không cần thay đổi nghiệp vụ ngoài việc sửa chú thích lỗi thời. Verification: 324 targeted BLL tests pass, 2/2 live MySQL tests pass, 3.416/3.416 non-MySQL tests pass, 127/127 frontend tests pass, solution build 0 warning/error, ESLint sạch, production build pass, `git diff --check` sạch. |
| **Checkpoint 8** | `feat(center-manager): complete dynamic rbac and authorization audit ux` | Phần 2 (H) | **COMPLETED (CORRECTIVE VERIFIED / TECHNICALLY APPROVED)** | Hoàn thiện toàn diện Giai đoạn H (Dynamic RBAC & Authorization Audit UX) kèm khắc phục triệt để các phát hiện audit từ Codex: (1) Chuẩn hóa tracking: gán đúng nhãn Giai đoạn H cho RBAC, bảo lưu Giai đoạn G ở trạng thái PENDING AUDIT; (2) Server-side search/filter/pagination cho danh sách vai trò (`page`, `pageSize`, `search`, `accountType`, `status`), loại bỏ giới hạn 100 bản ghi; (3) Bổ sung DTOs, UseCase `ListAuthorizationUsersUseCase` và endpoint `GET /api/v1/authorization/users` hỗ trợ tìm kiếm và phân trang toàn bộ người dùng trong trung tâm (`CenterManager` gồm cả secondary managers, `Teacher`, `Student`); (4) Read-only capability gating: cho phép tài khoản chỉ có quyền đọc (`authorization.user_roles.read`) duyệt danh sách người dùng, vô hiệu hóa mutation controls kèm banner thông báo rõ ràng; (5) Đồng bộ hóa selection khi chuyển trang: tự động căn chỉnh lựa chọn về người dùng đầu tiên của trang mới, loại bỏ nguy cơ lệch target ID giữa UI và API; (6) Phân trang và tìm kiếm vai trò tương thích trong UserRolePanel hỗ trợ không giới hạn số lượng vai trò (>100 roles) kèm cache `knownRoles` bảo toàn attribution; (7) Bảng Quyền hạn hiệu lực bảo toàn 100% quyền và nguồn vai trò ngay cả khi vai trò nằm ngoài trang 1 thông qua `userAuth.roles.permissionCodes` và cơ chế canonical fetching `authorizationApi.getRole`; (8) Bộ lọc H3 `PermissionCode` so khớp chính xác chuỗi JSON (`"code"`), loại bỏ hoàn toàn hiện tượng false-positive do trùng tiền tố/chuỗi con kèm bộ negative unit tests; (9) Giữ nguyên giờ/phút người dùng chọn cho `datetime-local` trong bộ lọc thời gian; (10) Đồng bộ validation frontend/backend: `roleCode` 1–64 ký tự (`^[A-Z][A-Z0-9_]*$`), `roleName` 1–150 ký tự; (11) Triển khai `parseSafeError` triệt tiêu rò rỉ raw `ProblemDetails.detail`/`title`, chỉ hiển thị thông báo an toàn mặc định và `traceId`; (12) Khắc phục triệt để 3 phát hiện corrective cuối cùng: gate `rolesQuery` theo `canReadRoles`, hiển thị trực tiếp `userAuth.roles` cho persona chỉ có `user_roles.read` tránh 403; lọc nghiêm ngặt `Permission.Status == PermissionStatus.Active` và `Role.Status == AuthorizationRoleStatus.Active` để hợp quyền breakdown đồng nhất tuyệt đối với `UserAuthorizationDto.Permissions`; tách các tiện ích hydration/breakdown thành shared production helpers (`authorizationManagementHelpers.ts`) dùng chung giữa component và test suite; (13) Backend 242/242 IdentityAndTenancy tests pass, frontend 118/118 tests pass, ESLint 0 warning/0 error, production build pass. |
| **Checkpoint 9** | `perf(web): split center admin routes and optimize bundle` | Phần 2 (I) | **COMPLETED (CORRECTIVE VERIFIED / TECHNICALLY APPROVED)** | Hoàn thành toàn diện Route-level Code Splitting và củng cố bảo mật điều hướng: (1) Áp dụng React.lazy/dynamic import cho toàn bộ 15 trang quản trị CenterManager và platform, bọc trong component chuẩn hóa RouteChunkBoundary với Accessible Suspense fallback và nút thử lại khi lỗi tải chunk; (2) Giảm kích thước bundle index chính từ 1,64 MB xuống 395,27 KB (giảm hơn 75%), loại bỏ hoàn toàn cảnh báo bundle > 500 KB của Vite; (3) Khóa chặt chẽ điều hướng AuthenticatedHomePage theo cả account type và capability (ngăn chặn lộ link platform, hỗ trợ tài khoản chỉ có quyền đọc hồ sơ); (4) Triệt tiêu rò rỉ raw ProblemDetails trên 11 trang quản trị; (5) Cập nhật kiểm thử rollback MySQL catalog lên 70 permissions chính xác; (6) 3.472/3.472 backend tests pass (3.416 non-MySQL + 56 live MySQL), 131/131 web tests pass, ESLint 0 warning/0 error, production build clean, EF model 0 drift. |
| **Checkpoint 10** | `test(center-manager): complete relational and live e2e verification` | Phần 2 (I) | **COMPLETED (VERIFIED)** | Xác thực toàn diện kiểm thử quan hệ và môi trường Live Docker: (1) 3.472/3.472 backend tests pass (3.416 non-MySQL + 56 live MySQL tests trên container port 3307); (2) 131/131 web tests pass; (3) EF Core model drift check: 0 pending model changes; (4) Khởi động và kiểm tra kết nối live stack Docker: Web (`http://localhost:3000`), API (`http://localhost:5000`), MySQL 8.0 (`localhost:3307`), Adminer (`localhost:8080`); (5) Kiểm thử xác thực live API bằng tài khoản mẫu CenterManager (`EDUTWIN_A / manager`), trả về HTTP 200, JWT token hợp lệ, Role SYSTEM_CENTERMANAGER, 57 permissions; (6) Kiểm thử Persona D ranh giới bảo mật: Request mang Bearer token CenterManager tới `/api/v1/platform/centers` bị từ chối với HTTP 403 Forbidden fail-closed. |
| **Checkpoint 11** | `test(center-manager): complete chrome e2e acceptance` | Phần 2 (I) | **COMPLETED (VERIFIED)** | Hoàn thành kiểm thử chấp nhận trình duyệt Chrome E2E bằng subagent tự động trên live Docker stack: (1) Đăng nhập tài khoản CenterManager `EDUTWIN_A / manager`; (2) Xác thực landing page `/` hiển thị đúng danh tính `Center Manager`, `EduTwin Center A` và không có bất kỳ liên kết Platform Admin nào; (3) Truy cập và kiểm tra `/quan-ly/trung-tam` (Hồ sơ trung tâm): `CenterCode` `EDUTWIN_A` và `Status` `Đang hoạt động` ở trạng thái read-only, hỗ trợ cập nhật OCC RowVersion; (4) Truy cập và kiểm tra `/quan-ly/phan-quyen` (Phân quyền & Vai trò): duyệt qua đầy đủ 3 tabs (`1. Vai trò & Ma trận quyền`, `2. Gán vai trò người dùng`, `3. Nhật ký kiểm toán phân quyền`), phân trang danh sách vai trò và người dùng trơn tru; (5) Thu thập và lưu trữ đầy đủ bằng chứng kiểm thử: Video recording `centermanager_live_e2e_1789481953404.webp` và các ảnh chụp màn hình `center_profile_page_1789482052187.png`, `roles_and_permissions_page_1789482072043.png`, `audit_logs_tab_1789481906858.png`, `user_roles_tab_1789481890479.png`. |
| **Checkpoint 12** | `docs(center-manager): publish operational completion closeout` | Phần 2 (I) | **COMPLETED (VERIFIED)** | Xuất bản báo cáo nghiệm thu kỹ thuật chính thức `docs/verification/POST_R09_CENTER_MANAGER_OPS_VERIFICATION_REPORT.md` tuân thủ đầy đủ 14 hạng mục nghiệm thu theo Mục 10 của kế hoạch `POST-R09-CENTER-MANAGER-OPS.md`; đối chiếu toàn bộ các gate kỹ thuật, số liệu kiểm thử, bundle metrics và bằng chứng Chrome E2E; chuyển giao milestone sang trạng thái: `POST-R09 CENTER MANAGER END-TO-END COMPLETION: TECHNICALLY VERIFIED / CHROME E2E PASS / UX ACCEPTANCE PENDING`. |

## 21. Student Subsystem Modernization & Integrated Math Assistants Tracking

Lộ trình thực hiện hiện đại hóa phân hệ học sinh và không gian trợ lý toán học tích hợp (STUDENT-SUBSYSTEM-MODERNIZATION):

| Giai đoạn | Nội dung thực thi | Trạng thái | Ghi chú & Bằng chứng |
|---|---|---|---|
| **Phase 1: Shell & Navigation** | Tạo `StudentLayout.tsx` với Top Navigation Bar, Subject selector đồng bộ hai chiều query params `?subjectId=...`, Streak indicator (`🔥 7 ngày liên tiếp`), ThemeToggle sáng/tối | **COMPLETED** | Gắn kết vào toàn bộ route `/hoc-tap/*` trong `App.tsx`. |
| **Phase 2: Bento Dashboard** | Hiện đại hóa `StudentDashboardPage.tsx` theo Bento Grid 4 KPIs (Target, Predicted, Days, Risk), Radar/Line charts, AI Recommendation Strategy banner | **COMPLETED** | Giữ trọn vẹn dữ liệu API TanStack Query, bổ sung nút Bỏ qua đề xuất (`btn-dismiss-recommendation`). |
| **Phase 3: Assignments & Detail** | Hiện đại hóa `StudentAssignmentsPage.tsx` & `StudentAssignmentDetailPage.tsx` với Search bar, Status filter tabs (`Tất cả`, `Chưa bắt đầu`, `Đang làm`, `Đã hoàn thành`, `Quá hạn`), animated progress bars | **COMPLETED** | Hiển thị chi tiết câu hỏi, mức độ nhận biết và trạng thái đã nộp kèm nút làm trực tiếp. |
| **Phase 4: Digital Twin & History** | Hiện đại hóa `StudentTwinPage.tsx` với Cognitive Growth & Velocity, Confidence Calibration Index, Knowledge Topics Table, Twin History Timeline | **COMPLETED** | Giữ trọn vẹn dòng thời gian biến động năng lực có phân trang và nhãn nguồn cập nhật (AI / Teacher). |
| **Phase 5: Learning Player & Math Assistants** | Dual-pane workspace tại `LearningPlayerPage.tsx` tích hợp `SideAssistantWorkspace` (Casio fx-580VN X, Vector Scratchpad, Grapher 2D, VisualMathField) | **COMPLETED** | Hỗ trợ đính kèm ảnh nháp PNG, chèn kết quả 1 chạm vào ô trả lời, hiển thị báo cáo tư duy đa phương thức AI. |
| **Phase 6: React Hooks Hardening** | Khắc phục triệt để lỗi vi phạm thứ tự React Hook (`Minified React error #310`) | **COMPLETED** | Đưa toàn bộ hooks (`useMemo`, `useState`, `useEffect`) lên đầu component, kiểm chứng build sạch sẽ. |
| **Phase 7: AI Multi-Key Pool** | Cấu hình cụm 3 khóa Gemini xoay tua Round-Robin kèm tự động chuyển khóa ngay lập tức (Automatic Failover) khi gặp lỗi quota hoặc sự cố mạng | **COMPLETED** | Cấu hình đồng bộ trong `docker-compose.yml`, `GeminiOptions.cs` và `GoogleGenAIGenerateContentClient.cs`. |

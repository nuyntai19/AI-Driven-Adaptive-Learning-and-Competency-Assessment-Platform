...
| 2026-09-12 | 3.0.3-gate2-completed | Gate 2: Track 1 — Platform Administration (Backend, Migrations, Frontend, Tests) | Hoàn thành toàn diện Gate 2 theo Plan v3.1.3-ADDENDUM-03: Thêm vai trò PlatformAdmin, cập nhật 5 MySQL CHECK constraints, sinh migration AddPlatformAdminSupportAndCheckConstraints, triển khai PlatformBootstrapOptions (fail-closed), PlatformAdminProvisioner idempotent, IPlatformCenterService và PlatformCenterService (atomic OCC, bulk token revocation, auth_version bump, target protection cấm mutate PLATFORM, audit invariant CenterId=PLATFORM và cross-tenant TargetUserId=null, empty state trả items: []), Controllers/PlatformCentersController, bảo vệ leo thang quyền hạn AuthPrivilegeEscalation, Frontend PlatformCentersPage.tsx với search/filter/create modal/status OCC modal/password reset modal, AppDrawer/AuthenticatedHomePage điều hướng, hoàn tất kiểm thử: dotnet test 3,078 passed, 0 failed, npm test 21 passed, 0 failed, npm run lint pass, npm run build pass. | Nhóm EduTwin | Gate 2 COMPLETED; 100% tests pass; 2 file DOCX bảo tồn nguyên vẹn |
...
| **GATE 2** | Track 1 — Platform Admin (5 CHECK constraints, Provisioner, Web UI) | **COMPLETED / READY TO FREEZE** | Đã triển khai đầy đủ 5 CHECK constraints, PlatformAdminProvisioner, IPlatformCenterService & PlatformCenterService với audit invariant (CenterId=PLATFORM, cross-tenant TargetUserId=null), PlatformCentersController, Frontend PlatformCentersPage.tsx và routing. 3,078/3,078 .NET tests pass (0 fail), 21/21 web tests pass (0 fail), eslint 0 errors, production build pass. |

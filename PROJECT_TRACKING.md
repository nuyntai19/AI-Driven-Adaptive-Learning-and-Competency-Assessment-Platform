...
| 2026-09-12 | 3.0.2-gate1-freeze | PROJECT_REQUIREMENTS, DATABASE_SCHEMA, docs/decisions/ADR-POST-R08-MATH.md, docs/decisions/ADR-POST-R08-PLATFORM.md, MASTER_PLAN, CONSTITUTION, API_CONTRACTS, PROJECT_TRACKING | Đóng băng chính thức Gate 1 (Gate 1 Frozen): Phân biệt lỗi AI/mạng/provider (deterministic fallback cho cả assignment lẫn free-practice) với sự cố hạ tầng AttachmentStorageUnavailable (chỉ bài free-practice cạn kiệt retry mới chuyển sang FailedTerminal/AnalysisFailed kèm luồng resubmit); đồng bộ schema đông cứng chính xác của Bảng 40 attempt_attachments (attachment_id, center_id, attempt_id, file_name, storage_key VARCHAR(512), file_size_bytes BIGINT, content_type, upload_nonce, created_at, created_by; xác thực SHA-256 in-memory qua signed token, không persist cột sha256_hash vào CSDL); bổ sung Platform Audit Invariant và verification cho PlatformAdmin cross-tenant (CenterId=PLATFORM, TargetUserId=null để bảo toàn composite tenant FK, định danh tại TargetId/redacted metadata, tuyệt đối không log password/secret). | Nhóm EduTwin | Gate 1 FROZEN; zero source code change; bảo tồn 2 file DOCX |

...
| **GATE 5** | Track 2D — Multipart Storage, Bảng vật lý 40, Gemini Multimodal & Review Fallback | **CORRECTIVE PENDING REVIEW** | Hoàn tất khắc phục toàn diện theo audit của Codex (P1 Permanent-Orphan Sweeper): `AttachmentOrphanCleanupWorker` có 1 constructor DI duy nhất, quét sạch permanent blob mồ côi quá 24h (`attempt-attachments`) không có DB reference bằng `IgnoreQueryFilters()`, fail-closed khi DB gặp sự cố, validate chuẩn hóa key `tenants/{centerId}/attempt-attachments/{nonce}.png`. Thống nhất đặc tả quyền tải file đính kèm `learning.attempts.read_scoped` + `IAttemptTeacherReviewScopeGuard` (khớp Section 52 & 52.2). Bằng chứng kiểm thử: Release build 0 warning/0 error; full suite 3,210/3,210 backend unit tests pass; 45/45 live MySQL integration tests pass trên port 3307; frontend `tsc -b` pass, 46/46 web tests pass, eslint 0 error, production build pass; EF model 0 drift. Chờ reviewer nghiệm thu đóng băng kỹ thuật chính thức. |
| **GATE 6** | Post-R08 Extension Verification & Hardening | **PENDING** | Clean-clone container rehearsal (worktree/clone cô lập tại freeze commit), EXPLAIN execution plans (đánh giá query trọng yếu và index thực tế), kiểm chứng E2E và lập báo cáo nghiệm thu R09 hoàn chỉnh. |

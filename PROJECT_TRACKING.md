...
| **Checkpoint 2** | `test(platform): enforce education-data denial matrix` | PENDING | Xây dựng bộ test suite tự động kiểm chứng từ chối truy cập dữ liệu học thuật của PlatformAdmin (HTTP 403 Fail-Closed). |
| **Checkpoint 3** | `feat(platform): add center manager lifecycle` | PENDING | Schema migration `primary_manager_user_id`, composite FK, IPlatformCenterService methods, API endpoints và UI modal quản lý nhân sự. |
| **Checkpoint 4** | `feat(platform): expose redacted platform audit history` | PENDING | Schema migration `target_center_id`, query service, DTO allow-list redaction, và UI `/quan-tri-nen-tang/nhat-ky`. |
| **Checkpoint 5** | `feat(platform): update center metadata and safe aggregates` | PENDING | Endpoint PATCH metadata, query safe counts (Student, Teacher, Class, Manager), cập nhật UI PlatformCentersPage. |
| **Checkpoint 6** | `feat(platform): harden suspension and platform self-security` | PENDING | Bắt buộc lý do khi tạm ngưng, thu hồi phiên toàn trung tâm, endpoints `/platform/me/*` và UI bảo mật tài khoản. |
| **Checkpoint 7** | `test(platform): complete relational and browser verification` | PENDING | Chạy full automated test suites, live MySQL tests, EF drift check, và 21 kịch bản kiểm thử Chrome thực tế. |
| **Checkpoint 8** | `docs(platform): publish operational enhancement closeout` | PENDING | Xuất bản báo cáo nghiệm thu kỹ thuật và cập nhật đóng lại milestone. |

...
public class ArchiveQuestionUseCase : IArchiveQuestionUseCase
{
    private readonly EduTwinDbContext _dbContext;
    private readonly ITenantContext _tenantContext;
    private readonly TimeProvider _timeProvider;

    public ArchiveQuestionUseCase(
    ...
    public async Task<ArchiveQuestionResult> ExecuteAsync(string questionId, ArchiveQuestionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(questionId) ||
            ...
            return ArchiveQuestionResult.Failure(ErrorCodes.ValidationFailed);
        ...
        foreach (var ch in request.RowVersion)
            if (ch < '0' || ch > '9') return ArchiveQuestionResult.Failure(ErrorCodes.ValidationFailed);
        ...
        if (question.RowVersion != rowVersion)
            return ArchiveQuestionResult.Failure(ErrorCodes.ConcurrencyConflict);
        ...
        return ArchiveQuestionResult.Success(QuestionProjection.ToDto(question, options, mappings));
    }
}

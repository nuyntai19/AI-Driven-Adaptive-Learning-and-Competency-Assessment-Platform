...
public class ListStudentsUseCaseTests
{
    ...
    [Fact]
    public async Task StudentListQuery_DoesNotExcludeClassMembers_ReturnsAllEligibleStudents()
    {
        var dbName = Guid.NewGuid().ToString();
        var dbContext = CreateContext(dbName);
        var someClassId = Guid.NewGuid();

        var s1Member = Guid.NewGuid();
        var s2NonMember = Guid.NewGuid();

        await SeedDataAsync(dbContext, s1Member, classId: someClassId, membershipStatus: ClassStudentStatus.Active);
        await SeedDataAsync(dbContext, s2NonMember);

        var sut = new ListStudentsUseCase(dbContext, _mockTenantContext.Object, _mockOwnershipGuard.Object);
        // StudentListQuery no longer has ExcludeClassId, calling with standard query returns all students
        var result = await sut.ExecuteAsync(new StudentListQuery());

        Assert.True(result.IsSuccess);
        Assert.Equal(2, result.Data!.Count);
        Assert.Contains(result.Data, s => s.StudentId == s1Member);
        Assert.Contains(result.Data, s => s.StudentId == s2NonMember);
    }
}

...
public class AssignCurriculumClassesUseCaseTests
{
    ...
    {
        var dbOptions = new DbContextOptionsBuilder<EduTwinDbContext>().UseInMemoryDatabase("AssignClasses_TenantNotResolved").Options;
        using var dbContext = new EduTwinDbContext(dbOptions);
        var tenantMock = new Mock<ITenantContext>();
        tenantMock.Setup(t => t.IsResolved).Returns(false);
        var timeProviderMock = new Mock<TimeProvider>();

        var sut = new AssignCurriculumClassesUseCase(dbContext, tenantMock.Object, timeProviderMock.Object);

        var result = await sut.ExecuteAsync(Guid.NewGuid(), new AssignCurriculumClassesRequest());

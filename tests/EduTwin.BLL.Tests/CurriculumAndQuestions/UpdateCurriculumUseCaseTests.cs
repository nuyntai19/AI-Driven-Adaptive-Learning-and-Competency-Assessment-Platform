...
public class UpdateCurriculumUseCaseTests
{
    ...
    {
        var dbOptions = new DbContextOptionsBuilder<EduTwinDbContext>().UseInMemoryDatabase("UpdateCurriculum_TenantNotResolved").Options;
        using var dbContext = new EduTwinDbContext(dbOptions);
        var tenantMock = new Mock<ITenantContext>();
        tenantMock.Setup(t => t.IsResolved).Returns(false);
        var timeProviderMock = new Mock<TimeProvider>();

        var sut = new UpdateCurriculumUseCase(dbContext, tenantMock.Object, timeProviderMock.Object);

        var result = await sut.ExecuteAsync(Guid.NewGuid(), new UpdateCurriculumRequest());
